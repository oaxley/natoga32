...
// abiRegisterNames maps register numbers to their ABI names
...
// RegisterName returns the name of a register given its number.
// If useABI is true, returns the ABI name (e.g., "ra", "sp", "a0").
// If useABI is false, returns the numeric name (e.g., "x1", "x2", "x10").
// Returns empty string if reg is out of range (0-31).
func RegisterName(reg int, useABI bool) string {
	if reg < 0 || reg > 31 {
		return ""
	}
	if useABI {
		return abiRegisterNames[reg]
	}
